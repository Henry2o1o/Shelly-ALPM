...
pub const variants = [_]types.Variant{
    ...
    .{
        ...
        .options = &.{ flag("--check", &.{}, "Run each PKGBUILD check() function during AUR upgrade builds"), flag("--no-check", &.{}, "Skip each PKGBUILD check() function during AUR upgrade builds"), flag("--singlepane", &.{}, "Use the shared pacman-style linear output (already the native Zig non-UI output mode)"), flag("--no-devel", &.{}, "Skip checking -git packages") },
    },
    .{
        .action = .upgrade,
        .name = "flatpak",
