...
pub const AtollAurPage = extern struct {
    ...
    const resource_path = "/com/shellyorg/shelly/ui/atoll_aur_page.ui";
    ...
    const Mode = enum { browse, search, installed };
    ...
    const LoadResult = struct {
        ...
        failed: bool = false,
        index_page: u32 = 0,
        index_total_pages: u32 = 0,
    };

    pub const getGObjectType = gobject.ext.defineClass(Self, .{
        .name = "ShellyAtollAurPage",
        .instanceInit = &init,
    ...
    fn set_state(self: *Self, state: State) void {
        ...
        switch (state) {
            ...
            .err => self.show_placeholder("dialog-error-symbolic", translations._("Could not reach Atoll"), translations._("Check your connection and try again.")),
        }
    }
    ...
    fn load_worker(page: *Self, generation: u64, mode: Mode) void {
        ...
        switch (mode) {
            .browse => {
                var atoll = AtollApiService.init(alloc, threaded.io());
                defer atoll.deinit();

                const requested = page.priv().current_page;
                const index = atoll.getIndexPage(
                    requested,
                    AtollApiService.page_size,
                    index_sort_by,
                    index_order,
                ) catch |err| {
                    std.debug.print("atoll_index failed: {t}\n", .{err});
                    post_failure(page, arena_ptr, generation);
                    return;
                };
                post_index_result(page, index, arena_ptr, generation);
            },
            .search => {
                var atoll = AtollApiService.init(alloc, threaded.io());
                defer atoll.deinit();
                ...
                post_result(page, packages, arena_ptr, generation);
            },
            .installed => {
                const cli = ShellyCli{ .allocator = alloc, .io = threaded.io() };
                const parsed = cli.list_aur_installed() catch |err| {
    ...
    fn post_result(
        ...
        post(page, packages, arena, generation, 0, 0);
    }

    fn post_index_result(
        page: *Self,
        index: AtollApiService.IndexPage,
        arena: *std.heap.ArenaAllocator,
        generation: u64,
    ) void {
        post(page, index.packages, arena, generation, index.page, index.total_pages);
    }
    ...
    fn post(
        ...
        result.* = .{
            ...
            .index_total_pages = index_total_pages,
        };
    ...
    fn onLoadComplete(data: ?*anyopaque) callconv(.c) c_int {
        ...
        if (result.index_page != 0) {
            p.current_page = result.index_page;
            p.total_pages = result.index_total_pages;
        }
        page.update_pager();
    ...
    fn update_pager(self: *Self) void {
        const p = self.priv();
        const browsing = p.mode == .browse;
        gtk.Widget.setVisible(p.pager_box.as(gtk.Widget), @intFromBool(browsing));
        if (!browsing) return;
        ...
        const label = std.fmt.bufPrintZ(
            &buf,
            "{s} {d} {s} {d}",
            .{ translations._("Page"), p.current_page, translations._("of"), p.total_pages },
        ) catch return;
        gtk.Label.setLabel(p.page_label, label);

        gtk.Widget.setSensitive(
            p.prev_page_button.as(gtk.Widget),
            @intFromBool(p.current_page > 1),
        );
        gtk.Widget.setSensitive(
            p.next_page_button.as(gtk.Widget),
            @intFromBool(p.total_pages == 0 or p.current_page < p.total_pages),
        );
    }

    fn on_prev_page(self: *Self) callconv(.c) void {
        const p = self.priv();
        if (p.current_page <= 1) return;
        p.current_page -= 1;
        self.start_load(.browse);
    }

    fn on_next_page(self: *Self) callconv(.c) void {
        const p = self.priv();
        if (p.total_pages != 0 and p.current_page >= p.total_pages) return;
        p.current_page += 1;
        self.start_load(.browse);
    }
    ...
    fn on_install_response(ctx: ?*anyopaque, confirmed: bool) void {
        const self: *AtollAurPage = @ptrCast(@alignCast(ctx.?));
    ...
    const template_children = .{
        ...
        .{ "pager_box", @offsetOf(Private, "pager_box") },
