...
pub const UnifiedPage = extern struct {
    parent_instance: Parent,

    const Self = @This();
    pub const Parent = gtk.Box;

    pub const title: [:0]const u8 = "Unified";
    pub const icon_name: [:0]const u8 = "arch-symbolic";
    const resource_path = "/com/shellyorg/shelly/ui/unified.ui";
    ...
    fn load_worker(page: *Self, generation: u64) void {
        ...
        const parsed = cli.get_packages(false) catch |err| {
        ...
        var aur_arena = std.heap.ArenaAllocator.init(std.heap.c_allocator);
        defer aur_arena.deinit();
        const aalloc = aur_arena.allocator();

        var ithreaded: std.Io.Threaded = .init(aalloc, .{});
        defer ithreaded.deinit();
        const acli = ShellyCli{ .allocator = aalloc, .io = ithreaded.io() };

        const aur = acli.search_aur("shelly") catch |err| {
            std.debug.print("get_packages failed: {t}\n", .{err});
            arena_ptr.deinit();
            std.heap.c_allocator.destroy(arena_ptr);
            return;
        ...
        // var flatpak_arena = std.heap.ArenaAllocator.init(std.heap.c_allocator);
        // defer flatpak_arena.deinit();
        // const falloc = flatpak_arena.allocator();

        // var fthreaded: std.Io.Threaded = .init(falloc, .{});
        // defer ithreaded.deinit();
        // const fcli = ShellyCli{ .allocator = falloc, .io = fthreaded.io() };

        // const flatpak = fcli.f("shelly") catch null;
        // _ = flatpak;

        post_result(page, aur.value, arena_ptr, generation);
    }
    ...
    fn on_install_response(ctx: ?*anyopaque, confirmed: bool) void {
        const self: *UnifiedPage = @ptrCast(@alignCast(ctx.?));
    ...
    fn on_install_clicked(self: *UnifiedPage) void {
        _ = self;
    }
